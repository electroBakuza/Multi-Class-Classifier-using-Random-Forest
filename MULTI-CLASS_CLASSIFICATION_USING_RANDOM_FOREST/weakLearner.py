...
class WeakLearner: # A simple weaklearner you used in Decision Trees...
    ...
    def evaluate_numerical_attribute(self,feat, Y):
        ...
        arr_abc=np.random.uniform(1,-1,(3,10))
        #step2:
        #def evaluate_numerical_attribute(self,feat, Y):
        
        one=np.ones( (feat.shape[1],1) )

        print one.shape
        print feat.shape
        sec=np.vstack( (feat,one.T ))
        s=np.dot( sec.T  , arr_abc)
        ...
        for t in range(s.shape[1]):
            tmp=s[:,t]
            score=self.calculateEntropy(Y,tmp>0)    
            if(score<ent):
                split=arr_abc[t]
                ent=score
                Xlidx=tmp>0
